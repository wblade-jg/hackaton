    ...
    <Box sx={{ px: { xs: 2, sm: 3 } }}>
      ...
      >
        ...
        <Box sx={{ minWidth: 0 }}>
          ...
          {fileInfo && (
            <Typography variant="body2" color="text.secondary">
              Procesado el {formatDate(fileInfo.processedDate)} &middot; {counts.total} transacciones
            </Typography>
          )}
        </Box>
      </Box>

      <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap', mb: 2.5 }} aria-label="Resumen de transacciones">
        <SummaryCard value={counts.total} label="Total" rule="primary.main" />
        <SummaryCard value={counts.processed} label="Procesadas" rule="success.main" />
        <SummaryCard value={counts.rejected} label="Rechazadas" rule="error.main" />
      </Box>
                      ...
                      visibleTransactions.map((tx, index) => (
                        <TableRow
                          ...
                          sx={{
                            ...
                            backgroundColor: tx.status === 'REJECTED' ? 'rgba(194, 52, 43, 0.045)' : undefined,
                          }}
